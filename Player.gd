...
## Función central para recibir mejoras o penalizaciones
func modificar_stat(stat_id: String, cantidad: float):
	match stat_id:
		"vida_max":
			max_health = max(10, max_health + cantidad) # Mínimo 10 de vida
			# Si curamos o el máximo subió, ajustamos la actual proporcionalmente
			if cantidad > 0:
		...
		"velocidad":
			# Modifica el multiplicador (0.1 = +10%)
			speed_multiplier = max(0.5, speed_multiplier + (cantidad / 100.0))
			
		"ataque":
			bonus_damage = max(0, bonus_damage + cantidad)

	stats_updated.emit()
	print("Stat modificado: ", stat_id, " | Nuevo valor: ", cantidad)
